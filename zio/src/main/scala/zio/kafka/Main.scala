...
case class Apps(producer: Producer, eventsConsumer: EventsConsumer) {
  ...
  def record(time: String): ProducerRecord[String, String] = new ProducerRecord("test", Random.nextString(3), s"$time ${Random.nextString(15)}")
  def pr(time: String) = Producer.produce[Any, String, String](record(time), Serde.string, Serde.string)

  def myApp = for {
    _ ← eventsConsumer.consume.fork
    _ ← printLine("Start kafka producer ...")
    _ ← Clock.currentDateTime.flatMap { time ⇒
      pr(time.toString)
